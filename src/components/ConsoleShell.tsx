...
export function ConsoleShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-void text-body">
      <header className="sticky top-0 z-20 border-b border-graphite bg-ink/95 backdrop-blur-sm">
        <div
          className={`mx-auto flex items-center justify-between gap-4 px-4 py-3 md:px-6 ${
            isLanding ? "max-w-6xl" : "max-w-[1280px]"
          }`}
        >
          ...
          <div className="flex items-center gap-3 text-right">
            ...
            {session ? (
              ...
              <Link href="/login" className="btn-ghost px-3 py-1 text-[10px]">
                Enter console
              </Link>
            ) : (
      ...
      <div className={`mx-auto flex flex-col md:flex-row ${isLanding ? "max-w-none" : "max-w-[1280px]"}`}>
        {isLanding ? null : (
          <nav
            aria-label="Console"
